...
import { Activity, AlertTriangle, Brain, Calendar, Heart, Hospital, MessageSquare, TrendingUp, Users, Zap, CheckCircle, Settings } from "lucide-react";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!apiResponse.ok || (data && typeof data === 'object' && (data.success === false || data.error))) {
        ...
        // Handle specific API key error with improved messaging
        if (errorMessage.includes("Incorrect API key provided") || errorMessage.includes("401")) {
          errorMessage = "API Configuration Issue: The system detected an API key mismatch. Please ensure you have either a valid OpenAI API key (starts with 'sk-') or Google API key (starts with 'AIzaSy') properly configured in your environment variables.";
        }
      ...
      toast.success("Analysis completed successfully", {
        description: "HEAL-EYE has processed your healthcare query with the available API configuration.",
        closeButton: true,
        duration: 3000
      });
        ...
        <Alert>
          ...
          <AlertDescription>
            <div className="space-y-2">
              <div className="font-semibold">API Configuration Status:</div>
              <div className="text-sm space-y-1">
                <div>• <strong>OpenAI API:</strong> Use API key starting with 'sk-' for GPT models</div>
                <div>• <strong>Google API:</strong> Use API key starting with 'AIzaSy' for Gemini models</div>
                <div>• The system will automatically detect and use the appropriate model based on your API key format</div>
              </div>
            </div>
          </AlertDescription>
        </Alert>
